...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowMinSize           = ImVec2(300,32);    // Minimum window size
...
void ImGui::ItemSize(const ImVec2& size, float text_ADDRESS_y)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    const float text_base_offset = ImMax(window->DC.CurrLineTextBaseOffset, text_ADDRESS_y);
...
void ImGui::ItemSize(const ImRect& bb, float text_ADDRESS_y)
{
    ItemSize(bb.GetSize(), text_ADDRESS_y);
}
...
//      ADDRESS_from_start_x == 0 : follow right after previous item
//      ADDRESS_from_start_x != 0 : align to specified x position (relative to window/group left)
//      spacing_w < 0            : use default spacing if pos_x == 0, no spacing if pos_x != 0
//      spacing_w >= 0           : enforce spacing amount
void ImGui::SameLine(float ADDRESS_from_start_x, float spacing_w)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImGuiContext& g = *GImGui;
    if (ADDRESS_from_start_x != 0.0f)
    {
        if (spacing_w < 0.0f) spacing_w = 0.0f;
        window->DC.CursorPos.x = window->Pos.x - window->Scroll.x + ADDRESS_from_start_x + spacing_w + window->DC.GroupOffset.x + window->DC.ColumnsOffset.x;
...
static void ImGui::NavUpdate()
{
    ...
    float nav_scoring_rect_ADDRESS_y = 0.0f;
    if (nav_keyboard_active)
        nav_scoring_rect_ADDRESS_y = NavUpdatePageUpPageDown(allowed_dir_flags);
    ...
    g.NavScoringRectScreen.TranslateY(nav_scoring_rect_ADDRESS_y);
...
static float ImGui::NavUpdatePageUpPageDown(int allowed_dir_flags)
{
    ...
    if (page_up_held != page_down_held || home_pressed != end_pressed) // If either (not both) are pressed
    {
        ...
        else
        {
            ...
            const float page_ADDRESS_y = ImMax(0.0f, window->InnerRect.GetHeight() - window->CalcFontSize() * 1.0f + nav_rect_rel.GetHeight());
            float nav_scoring_rect_ADDRESS_y = 0.0f;
            if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
            {
                nav_scoring_rect_ADDRESS_y = -page_ADDRESS_y;
            ...
            return nav_scoring_rect_ADDRESS_y;
        }
    }
    return 0.0f;
}
