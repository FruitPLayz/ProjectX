...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const float text_base_ADDRESS_y = ImMax(0.0f, window->DC.CurrLineTextBaseOffset); // Latch before ItemSize changes it
    ...
    RenderText(bb.Min+ImVec2(g.FontSize + style.FramePadding.x*2, text_base_ADDRESS_y), text_begin, text_end, false);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const float inputs_ADDRESS_x = (style.ColorButtonPosition == ImGuiDir_Left) ? w_button : 0.0f;
    window->DC.CursorPos.x = pos.x + inputs_ADDRESS_x;
    ...
    {
        const float button_ADDRESS_x = ((flags & ImGuiColorEditFlags_NoInputs) || (style.ColorButtonPosition == ImGuiDir_Left)) ? 0.0f : w_inputs + style.ItemInnerSpacing.x;
        window->DC.CursorPos = ImVec2(pos.x + button_ADDRESS_x, pos.y);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    const float text_ADDRESS_x = g.FontSize + (display_frame ? padding.x*3 : padding.x*2);               // Collapser arrow width + Spacing
    const float text_ADDRESS_y = ImMax(padding.y, window->DC.CurrLineTextBaseOffset);                    // Latch before ItemSize changes it
    const float text_width = g.FontSize + (label_size.x > 0.0f ? label_size.x + padding.x*2 : 0.0f);    // Include collapser
    ImVec2 text_pos(window->DC.CursorPos.x + text_ADDRESS_x, window->DC.CursorPos.y + text_ADDRESS_y);
    ItemSize(ImVec2(text_width, frame_height), text_ADDRESS_y);
    ...
    if (!is_leaf)
    {
        if (pressed)
        {
            ...
            if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
                toggled |= IsMouseHoveringRect(interact_bb.Min, ImVec2(interact_bb.Min.x + text_ADDRESS_x, interact_bb.Max.y)) && (!g.NavDisableMouseHover);
            if (flags & ImGuiTreeNodeFlags_OpenOnDoubleClick)
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            RenderBullet(window->DrawList, ImVec2(text_pos.x - text_ADDRESS_x * 0.60f, text_pos.y + g.FontSize * 0.5f), text_col);
        else if (!is_leaf)
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_ADDRESS_x + padding.x, text_pos.y), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f);
        else // Leaf without bullet, left-adjusted text
            text_pos.x -= text_ADDRESS_x;
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            RenderBullet(window->DrawList, ImVec2(text_pos.x - text_ADDRESS_x * 0.5f, text_pos.y + g.FontSize * 0.5f), text_col);
        else if (!is_leaf)
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_ADDRESS_x + padding.x, text_pos.y + g.FontSize * 0.15f), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.70f);
        if (g.LogEnabled)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    // Compute width
    const float initial_ADDRESS_x = 0.0f; // g.Style.ItemInnerSpacing.x;
    const float width_avail = ImMax(tab_bar->BarRect.GetWidth() - initial_ADDRESS_x, 0.0f);
    ...
    float ADDRESS_x = initial_ADDRESS_x;
    float ADDRESS_x_ideal = ADDRESS_x;
    tab_bar->OffsetNextTab = ADDRESS_x; // This is used by non-reorderable tab bar where the submission order is always honored.
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ImGuiTabItem* tab = &tab_bar->Tabs[tab_n];
        tab->Offset = ADDRESS_x;
        if (scroll_track_selected_tab_id == 0 && g.NavJustMovedToId == tab->ID)
            scroll_track_selected_tab_id = tab->ID;
        ADDRESS_x += tab->Width + g.Style.ItemInnerSpacing.x;
        ADDRESS_x_ideal += tab->WidthContents + g.Style.ItemInnerSpacing.x;
    }
    tab_bar->OffsetMax = ImMax(ADDRESS_x - g.Style.ItemInnerSpacing.x, 0.0f);
    tab_bar->OffsetMaxIdeal = ImMax(ADDRESS_x_ideal - g.Style.ItemInnerSpacing.x, 0.0f);
...
float ImGui::GetColumnOffsetFromNorm(const ImGuiColumns* columns, float ADDRESS_norm)
{
    return ADDRESS_norm * (columns->OffMaxX - columns->OffMinX);
}
...
static float GetColumnWidthEx(ImGuiColumns* columns, int column_index, bool before_resize = false)
{
    ...
    float ADDRESS_norm;
    if (before_resize)
        ADDRESS_norm = columns->Columns[column_index + 1].OffsetNormBeforeResize - columns->Columns[column_index].OffsetNormBeforeResize;
    else
        ADDRESS_norm = columns->Columns[column_index + 1].OffsetNorm - columns->Columns[column_index].OffsetNorm;
    return ImGui::GetColumnOffsetFromNorm(columns, ADDRESS_norm);
}
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    float ADDRESS_0 = GetColumnOffset(columns->Current);
    float ADDRESS_1 = GetColumnOffset(columns->Current + 1);
    float width = ADDRESS_1 - ADDRESS_0;
    PushItemWidth(width * 0.65f);
    window->DC.ColumnsOffset.x = ImMax(column_padding - window->WindowPadding.x, 0.0f);
    window->DC.CursorPos.x = (float)(int)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x);
    window->WorkRect.Max.x = window->Pos.x + ADDRESS_1 - column_padding;
}
