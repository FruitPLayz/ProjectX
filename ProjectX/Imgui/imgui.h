...
struct ImVec4
{
    ...
    inline ImVec4 Darken(float fTransparant = 1.0f)
    {
        ImVec4 vNew = ImVec4(x + 10.0f, y + 10.f, z + 10.0f, fTransparant);
        if (vNew.x > 255.0f) vNew.x = 255.0f;
        if (vNew.y > 255.0f) vNew.y = 255.0f;
        if (vNew.z > 255.0f) vNew.z = 255.0f;
        return vNew;
    }

    inline ImVec4 Transparant(float fTransparant = 1.0f)
    {
        ImVec4 vNew = ImVec4(x, y, z, fTransparant);
        return vNew;
    }
};
...
namespace ImGui
{
    ...
    IMGUI_API void          SameLine(float ADDRESS_from_start_x=0.0f, float spacing=-1.0f);  // call between widgets or groups to layout them horizontally. X position given in window coordinates.
    ...
    IMGUI_API void          SetColumnOffset(int column_index, float ADDRESS_x);                  // set position of column line (in pixels, from the left side of the contents region). pass -1 to use current column
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags__OptionsDefault = ImGuiColorEditFlags_Float|ImGuiColorEditFlags_DisplayRGB|ImGuiColorEditFlags_InputRGB| ImGuiColorEditFlags_PickerHueWheel | ImGuiColorEditFlags_NoInputs| ImGuiColorEditFlags_NoTooltip,
